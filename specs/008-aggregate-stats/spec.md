...
- **FR-018**: The report's **per-player breakdown** MUST include, for each player (canonical identity, attended players only): season total goals and total assists, each followed by the corresponding per-attended-game rate in brackets (e.g. `12 goals (2.2 p/g)`), plus food-tracking % of attended games and weight-loss % of attended games — using the attended-games denominator defined in FR-007/FR-008/FR-010. In the human-readable block these fields are separated by emojis rather than punctuation.
