...
- [X] T024 [P] [US4] Implement `formatReportBlock` (chat-safe: plain `Label: value` lines for the team section under a `Players:` heading + one line per attended player showing season totals with the per-attended-game rate in brackets and emoji-separated fields — `- Name — ⚽ 12 goals (2.2 p/g) 🅰️ 6 assists (1.1 p/g) 🍽️ food tracking 60% 📉 weight-loss 60%`; **no fixed-width columns, no box-drawing, no ANSI, no pager** — FR-016/FR-018) and `formatReportJSON` in `src/cli/output/aggregate-formatters.ts`. Depends on T002.
