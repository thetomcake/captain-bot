...
**Phase 4 fix (2026-06-12) — poll vote decryption**: Daemon connected and scheduled correctly but recorded zero poll votes. Root cause was two missing Baileys requirements in `src/whatsapp/client.ts`, both confirmed against the Baileys docs (see research.md → WhatsApp Integration → "Poll vote decryption requirements"): (1) `makeWASocket` was not given a `getMessage` callback, which Baileys requires to recover a poll's `messageSecret` and decrypt incoming votes; (2) the poll-creation message was never in the message store, because the `messages.upsert` handler discarded everything that wasn't `type === 'notify'` — but a poll we send is echoed back as `type === 'append'`. Fix: store every upserted message regardless of `type` (domain dispatch still gated on `notify`), store the message returned by `sendMessage` directly, and wire `getMessage` to that store. The `MessageCounterError` seen at startup is unrelated libsignal replay-protection noise during offline sync, not a fault. LID groups surface voter JIDs as `@lid` — decryption is unaffected but JID-keyed user records should treat `@lid`/`@s.whatsapp.net` as the same person (follow-up, not part of this fix).
