...
- The scraped fixtures cover the **whole league**, not just our team. When picking the next fixture for a poll, the system selects our team's (`TEAM_NAME`) next upcoming game — where our team is home **or** away — not the earliest league fixture, and shows the other side as the opponent (FR-002a). If the scrape returns league fixtures but none feature our team (e.g. off-season for us, or a `TEAM_NAME` that doesn't match the site's spelling), the system treats it as "no confirmed next fixture" (FR-028) and logs that league fixtures were present but none matched our team — a likely `TEAM_NAME` mismatch.
...
- **FR-002a**: The club fixtures page lists the **whole league** (every team's games), not just ours. The system MAY store all scraped league fixtures (retained for future use), but when selecting the **next fixture** for a poll (FR-012) it MUST select our configured team's (`TEAM_NAME`) next upcoming game — the next game in which our team appears as **either** the home or the away side — never merely the earliest league fixture. Team-name matching is case-insensitive and whitespace-trimmed. The displayed opponent MUST be the **other** side (the team that is not ours), correct whether we are home or away. The poll wording and venue MUST NOT change based on home/away — fixtures are always at the same place. See `plan-next-fixture-selection.md`.
