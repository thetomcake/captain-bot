...
- [X] T023 [P] [US2] Write `tests/unit/whatsapp/poll-presenter.test.ts` (replaces the deleted poll-manager test): pure poll question/options formatting from a fixture. **Done:** asserts `formatPollQuestion` includes opponent + human date and reads as a question, `getPollOptions` returns a fresh `['Yes','No','Maybe']` (defensive copy), and `buildPollSpec` produces the single-choice `PollSpec`.
- [X] T024 [P] [US2] Rewrite `tests/integration/whatsapp/poll-service.test.ts` against `FakeGateway`: `sendPoll` persists the keyset (`messageSecret`+`groupId` onto the poll row, `pollId`→`pollMessageId`, exact `options`); `onPollVote` upserts/deletes `poll_responses` by canonical identity; vote-change overwrites, withdrawal (`selectedOptions: []`) deletes, two address forms collapse to one row (FR-013/SC-008); tally read back from DB rows (acceptance scenario 2). **Done:** fixtures come only from `manvfat-fixtures.html` via `MockFixtureScraper` (no real fetch); also covers unknown-poll-id ignore + replacement (FR-027) incl. delete-failure warning path.
- [X] T025 [P] [US2] Update `tests/integration/cli/poll-command.test.ts` for `--dry-run` (re-fetch + preview, send nothing, exit 0), missing `AUTHORIZED_GROUP_ID` (exit 3), **no confirmed next fixture after re-fetch → exit 1** (FR-028), and `--force` replacement (FR-027). **Done:** all fetches go through `MockFixtureScraper(manvfat-fixtures.html)`; the no-fixture case uses a placeholder-only static HTML snippet (mirrors the file's "Fixtures to be confirmed" rows — no real fetch); also asserts happy-path post (exit 0) and existing-poll-without-`--force` (exit 2).

### Implementation for User Story 2

- [X] T026 [P] [US2] Create `src/whatsapp/poll-presenter.ts` (de-Baileyed poll question/options formatting lifted from the deleted `poll-manager.ts`) to pass T023. **Done:** pure `formatPollQuestion`/`getPollOptions`/`buildPollSpec` over a `Game`; touches only the port's `PollSpec`, no client/DB.
- [X] T027 [US2] Implement the keyset store `src/whatsapp/keyset-store.ts`: `persist(keyset)` writes `messageSecret`+`groupId` onto the poll row at `sendPoll` time; `resolve(ref)` reconstructs `{ pollId: pollMessageId, groupId, messageSecret, options: pollOptions }` by `pollMessageId == ref.pollId AND groupId == ref.groupId`, or `null` if absent/replaced (FR-012/FR-014). Wire it into `gateway-factory.ts` `resolvePollKeyset` (T015). **Done:** `KeysetStore.persist({ gameId, question, postedAt, keyset })` writes the new poll row carrying its keyset; `resolve(ref)` returns the reconstructed `PollKeyset` or `null`; the factory's inline resolver was removed and now delegates to `keysetStore.resolve`.
- [X] T028 [US2] Rework `src/services/poll-service.ts` onto `IWhatsAppGateway`: build the `PollSpec` via the presenter, `sendPoll(group, spec)`, persist the poll row + keyset (T027); on `onPollVote` resolve the user via `whatsapp_users.canonicalId` (get-or-create) and persist each delta immediately as a replace-by-voter update of `poll_responses` (upsert, or delete on empty selection) — DB rows are the source of truth, never the Gateway's in-memory `aggregateVotes` (FR-013, research §3a). Also add a shared **orchestration** method `postOrReplaceNextPoll()` (reused by both the `!postpoll` handler and the `poll` CLI): re-fetch fixtures on demand via `fixture-service` (FR-003) → pick the next confirmed fixture; return a structured outcome `{ outcome: 'posted' | 'replaced' | 'no-fixture' | 'fetch-failed', ref? , fixture? }` — `no-fixture` when the scrape yields no confirmed next fixture (placeholder skipped), `fetch-failed` on scrape error; otherwise post (or replace an existing poll via T029). **Done.** Note: the outcome union was extended with an `'exists'` variant (existing poll + no `--force`) so the `poll` CLI can refuse without re-sending (exit 2); the `!postpoll` path always forces, so it never produces `'exists'`. Added `previewNextPoll()` for the `--dry-run` path (re-fetch, no send).
- [X] T029 [US2] Implement poll replacement in `poll-service.ts` (FR-027): on re-trigger / `--force`, hard-delete the poll's `poll_responses` then the `polls` row, then best-effort `deleteMessage(ref)` — on `{ ok: false }` log a timestamped warning and continue, never block. (This is also the manual reschedule path: a human re-sends `!postpoll` after a fixture changes — no automatic reschedule detection, FR-026.) **Done:** `removeExistingPoll` cascades responses→poll row→best-effort `gateway.deleteMessage` (which never throws — checks `outcome.ok`, warns on failure); send happens before any DB mutation so a `sendPoll` failure leaves the prior poll intact.
- [X] T030 [US2] Rework `src/cli/commands/poll.ts` on the port (`contracts/cli-commands.md`) as the **admin escape hatch** for `!postpoll`: call `poll-service.postOrReplaceNextPoll()` (T028); `--dry-run` re-fetches + previews (send nothing, exit 0); default requires `AUTHORIZED_GROUP_ID` (exit 3); `no-fixture`/`fetch-failed` → print why, exit 1 (FR-028); refuses an existing poll unless `--force` (FR-027); prints the poll ref; `--json`. Make T024/T025 pass. **Done:** signature is now `pollCommand(options, deps?)` where `deps` injects a fake Gateway + static-HTML `FixtureService` for tests (prod builds the real Gateway via `createGateway` and connects); dropped the unused `[game-id]` arg (the orchestration always targets the next fixture) and updated the `cli/index.ts` route accordingly; existing-poll-without-`--force` → exit 2.
- [X] T049 [P] [US2] Write `tests/integration/whatsapp/postpoll-trigger.test.ts` (write first, verify failing) driving `FakeGateway.simulateMessage` through the poll-trigger handler: (a) a message equal to `!postpoll` (case-insensitive, trimmed) re-fetches fixtures and posts a poll for the next fixture, recorded in `sentPolls`, **no** in-chat reply on success; (b) a second `!postpoll` hard-deletes the prior poll + responses and posts a fresh one (FR-027); (c) when the scrape yields no confirmed next fixture (placeholder skipped) the handler posts no poll and sends an in-chat reply via `sendMessage` (recorded in `sentMessages`); (d) on scrape failure, no poll + an in-chat error reply (FR-028); (e) a message merely containing the words "post poll" is ignored (treated as ordinary chat, FR-029). Use `FakeGateway` + the fake scraper; placeholder rows already exist in `tests/fixtures/html/manvfat-fixtures.html`. **Done:** the handler is wired to `onMessage` exactly as the event-router (T035) will (gate on `isPostPollCommand` first); happy/replace paths use `MockFixtureScraper(manvfat-fixtures.html)`, no-fixture uses a placeholder-only static HTML snippet, fetch-failure uses `ErrorMockScraper` — no real fetch anywhere. Also unit-tests `isPostPollCommand` matching/rejection.
- [X] T050 [US2] Implement the `!postpoll` trigger handler `src/whatsapp/postpoll-trigger.ts` (FR-029): export `isPostPollCommand(text)` (whole-message exact match of `!postpoll`, case-insensitive, trimmed) and `handlePostPoll(message)` which calls `poll-service.postOrReplaceNextPoll()` (T028), stays silent in-chat on `posted`/`replaced`, and on `no-fixture`/`fetch-failed` replies in the authorized group via `gateway.sendMessage` ("no confirmed next fixture yet" / "couldn't reach the club site") and logs every outcome with a timestamp (FR-025/FR-028). The event-router (T035) must call `isPostPollCommand` **first** and route to `handlePostPoll`, bypassing stat extraction. Make T049 pass. **Done:** exports `isPostPollCommand` + `createPostPollHandler(deps)` returning the bound `handlePostPoll(message)` (deps = `pollService`/`gateway`/`groupId`); always `postOrReplaceNextPoll({ force: true })` (re-trigger replaces, FR-027); logs every outcome. Event-router wiring itself lands in T035 (Phase 5).
- [X] T051 [US2] **(retrospective — raised in code review)** Add a 5-minute `!postpoll` throttle to defuse the FR-029 spam-replace footgun (any member can re-trigger a hard-delete of the poll + votes). New nullable `teams.last_poll_posted_at` column (folded into the existing migration `20260615152442_milky_meggan` + snapshot — pre-release, no separate migration) records the last post/replace time; `PollService.postOrReplaceNextPoll` stamps it on success (`recordPollPosted`) and exposes `getLastPollPostedAt()`. `createPostPollHandler` gates on it: a trigger arriving within `POSTPOLL_MIN_INTERVAL_MS` (5 min, injectable via `minIntervalMs` for tests) is **ignored** — silent in-chat, only logged. The `poll` CLI (admin escape hatch) is **exempt** (no throttle check), though a CLI post still advances the window. **Done:** covers both the within-window ignore and the post-window replacement (tests backdate `last_poll_posted_at`); does not fully close a truly-simultaneous two-message race (the `unique(gameId)` constraint remains the backstop) — see `future-todos.md` §1.
...
- [X] T031 [P] [US3] Write `tests/unit/stats/stat-extractor.test.ts`: pure regex + confidence scoring — `"2 goals, 1 assist, weight down, tracked food"`→{2,1,down,yes}; `"scored today"`→1 goal; `"great game everyone"`→below threshold (no capture); uncertainty markers ("think/maybe/probably") subtract confidence below 70% (FR-016/FR-018, research §6). **Done:** also asserts `CONFIDENCE_THRESHOLD === 70`, numeric goals/assists, single-field correction (`"correction 1 goal"`), weight direction up/down/same only (no value/BMI leak, FR-021), food yes/no, and confidence 0 for empty/non-stat text.
- [X] T032 [P] [US3] Write `tests/integration/stats/stat-capture.test.ts` against `FakeGateway`: within-window capture attributed to `m.sender.canonicalId`; 4+ days→treated as chat, not captured; first message applies defaults (goals=0/assists=0/weight=unknown/tracking=no); later partial message merges only mentioned fields; an explicit correction overrides only the named field (after "2 goals, 2 assists", a later "correction 1 goal" → goals=1, assists stay 2); WhatsApp edits/deletes ignored (FR-017/FR-019/FR-020/FR-024). **Done:** drives `FakeGateway.simulateMessage` through the real `registerEventRouter` (T035) with a stub `handlePostPoll`; also asserts casual-chat below-threshold non-capture, `!postpoll` routed to the poll handler (not captured as a stat, FR-029), and two address forms (`alice`/`aliceLid`) collapsing to one stat row (SC-008). Edits/deletes are not a separate path: the Gateway surfaces only genuine inbound messages, so the player-driven override IS the only mutation path (covered by the merge/correction cases).

### Implementation for User Story 3

- [X] T033 [P] [US3] Implement the pure extractor `src/stats/stat-extractor.ts` (regex + 0–100 confidence, ≥70% threshold; weight direction only — no weight/BMI values, FR-021) to pass T031. **Done:** `extractStats(text)` returns `ExtractedStats` with only the mentioned fields set (so the service can merge field-by-field) and `confidence` = strongest matched signal minus a one-shot `UNCERTAINTY_PENALTY` (25) when hedging markers appear; exports `CONFIDENCE_THRESHOLD = 70`. No I/O, no WhatsApp coupling.
- [X] T034 [US3] Implement `src/services/stat-service.ts` capture/merge: resolve sender via `whatsapp_users.canonicalId` (get-or-create), enforce the 3-day window against the relevant game, apply defaults on first capture, merge mentioned fields on subsequent messages, write `stat_records` (FR-015/FR-019/FR-020). **Done:** `captureFromMessage(message)` gates on confidence (`>= CONFIDENCE_THRESHOLD`) then the window (`findRelevantGame` picks the most recent game with `gameDate` in `[ts-3d, ts]`, keyed off `message.timestamp`), upserts `stat_records` by `unique(gameId, userId)` (defaults overlaid on insert, only-mentioned-fields on merge), and logs each capture (FR-025). Exports `STAT_WINDOW_DAYS = 3`.
- [X] T035 [US3] Create the event router `src/whatsapp/event-router.ts` (reworks the deleted `message-handler.ts`): on `onMessage`, **first** check `isPostPollCommand(m.text)` (T050) and route to `handlePostPoll`, returning before any stat parsing (FR-029); otherwise route to stat capture (text + window guard); route `onPollVote` to the poll tally (T028). Make T032 pass (and keep T049 green). **Done:** `registerEventRouter({ gateway, statService, pollService, handlePostPoll })` wires `onMessage` (postpoll-first, else `statService.captureFromMessage`) and `onPollVote`→`pollService.handlePollVote`, each wrapped in try/catch that logs and swallows so one bad event never tears down the listener. The daemon (T045) is the production caller. T032 green; full suite 205/205.
...
- [X] T041 [P] [US5] Write `tests/integration/seasons/season-transition.test.ts`: when all previously scraped fixtures disappear on re-scrape (fake scraper), `shouldCreateNewSeason()` returns true → end current season, create next, new fixtures populate it, prior season/games/polls/stats retained (FR-004/FR-005, SC-006/SC-007). **Done:** uses an inline `StubScraper` returning controlled `Fixture[]` for full control; asserts the four `shouldCreateNewSeason` branches (first-scrape false, partial-overlap false, empty-scrape false, all-gone true) and the sync-path transition (two seasons; old ended + `endDate` set + games/poll/stat retained; new current with only the new fixtures; no cross-contamination) plus the no-op re-sync case.

### Implementation for User Story 5

- [X] T042 [US5] Implement `SeasonService.shouldCreateNewSeason()` in `src/services/season-service.ts` (currently a `return false` placeholder): true iff every previously scraped fixture is absent from the latest scrape (research §7); reuse existing `endSeason`/`createNewSeason`, toggling `isCurrent` only (no cascade delete). **Done:** new signature `shouldCreateNewSeason(teamId, scrapedFixtures)`; "previously scraped fixtures" = the current season's **upcoming** games (past/completed fixtures drop off the site naturally, so including them would force a false transition every sync); matches by `opponent|YYYY-MM-DD` (local date key mirroring the scraper's parsing). Guards against false positives (SC-006): returns false on empty scrape (transient/off-season), no current season, or nothing previously scraped. Transition orchestration (end+create) is driven by the sync path (T043) reusing `createNewSeason` (which calls `endSeason` — `isCurrent` toggle, no cascade delete).
- [X] T043 [US5] Wire transition detection into the `sync` path (`src/services/fixture-service.ts` / `src/cli/commands/sync.ts`) so a manual re-scrape can trigger a season transition; make T041 pass. **Done:** `syncFixtures` now scrapes **once**, calls `shouldCreateNewSeason`, and on a transition calls `createNewSeason` (logs the FR-025 audit line) before persisting the new fixtures into the now-current new season; returns a `SyncResult { games, seasonTransition, newSeasonNumber }`. Extracted a private `persistScrapedFixtures`/`getTeam` so `fetchFixtures` and `syncFixtures` share the store logic without double-scraping. `sync.ts` reports the transition then the fixture count. `poll-service.ts`'s `!postpoll` re-fetch also uses `syncFixtures`, so FR-005 detection runs on that path too (it ignores the return value). T041 green; full suite 220/220, < 10 s (SC-010).
...
- [X] T044 [US setup] Rewrite `src/cli/commands/connect.ts` on `connect()` + `listGroups()`: render the `onQR` value as a terminal QR (`qrcode-terminal`) **and** a saved PNG (`qrcode.toFile`, print path) — FR-007; print groups table `id  [addressingMode]  name` and the `AUTHORIZED_GROUP_ID` hint (printed only, FR-011); `--reset`; exit `0`. **Done (with adaptation):** per user instruction the saved PNG path is **printed AND best-effort opened** (`xdg-open` on Linux / `open` on macOS) via a shared `src/cli/output/qr.ts` `renderQr` helper (reused by the daemon, T045). Built in `discovery: true` mode (no `AUTHORIZED_GROUP_ID` needed). `--reset` discards the stored snapshot first via a new `CredentialsStore.clear(teamId)` so a fresh QR pairs; disconnects after listing; exit `0` on success, `4` on connection failure.
- [X] T045 Rework `src/cli/commands/daemon.ts` as a **pure event listener — NO crons** (`croner` removed, T001): require `AUTHORIZED_GROUP_ID` (exit 3); build the Gateway via the factory; subscribe `onConnectionChange` (log only, FR-010), `onMessage`→event-router (T035: `!postpoll` trigger first, else stat capture), `onPollVote`→poll tally; keep the process alive with no scheduled jobs (all poll posting + fixture fetching is trigger/`sync`-driven, FR-003/FR-012/FR-029); on `SIGINT`/`SIGTERM` persist credentials via `getCredentials()`, `disconnect()`, exit `0`. **Once the cron job and `croner` import are gone from this file, run `npm uninstall croner`** to finish the dependency removal deferred from T001 (it is the last `croner` consumer). **Done:** checks `AUTHORIZED_GROUP_ID` env first (exit 3), team (exit 2); builds the Gateway via `createGateway({ db })`, wires `registerEventRouter` (T035) + `createPostPollHandler` (T050) + `onConnectionChange` (log only) + `onQR` (shared `renderQr`); no scheduled jobs — the open socket keeps the process alive; `SIGINT`/`SIGTERM` persists `gateway.getCredentials()` then `disconnect()` + exit `0`. **`npm uninstall croner` run** — `package.json`/`package-lock.json` no longer reference `croner` (T001 deferral closed).
- [X] T046 [P] Audit `src/utils/logger.ts` usage for the FR-025 timestamped audit trail (polls posted, messages processed, fixtures checked, connection-state changes, errors) across services/commands. **Done:** confirmed coverage — polls posted/replaced (`poll-service` + `postpoll-trigger`), stat captures (`stat-service`), errors (`event-router` swallow-and-log, `poll`/`postpoll`), and connection-state changes (now logged via `onConnectionChange` in both `daemon` and `connect`). One gap closed: added a `'Fixtures checked'` audit line in `FixtureService.syncFixtures` (scraped/persisted counts + transition flag) — previously only the season-transition branch logged.
- [X] T047 [P] Update `CLAUDE.md` and any README pointers to reflect the Gateway-native architecture (seam + ports), removing references to the deleted direct-Baileys modules. **Done:** `CLAUDE.md` had no stale Baileys references (it only points to `plan.md`); there is no project-root README. Updated `tests/README.md` "WhatsApp Tests" to describe the `FakeGateway`/`IWhatsAppGateway` seam (replacing the deleted `IWhatsAppClient`/`MockWhatsAppClient` example) and the SC-011 no-Baileys guard.
- [X] T048 Run the full suite (`npm test`) and confirm < 10 s (SC-010), all green, SC-011 guard passing; then walk the `quickstart.md` per-story validation (automated gate) and the manual `connect`/`daemon` interactive paths. **Done (automated gate):** `npm run build` (`tsc`) passes clean — the build works after Phase 8; full suite **220/220 green in ~6.6 s** (< 10 s, SC-010); SC-011 no-Baileys guard green. Manual `connect`/`daemon` interactive WhatsApp paths remain a human step per the ratified Constitution II exclusion (QR pairing / live votes are validated outside the automated suite).
