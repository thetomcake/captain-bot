...
- [ ] T044 [US5] Implement `deleteMessage(ref)` in `src/whatsapp-gateway/gateway.ts`: guards via `requireConnected`; send `{ delete: { remoteJid, fromMe: true, id } }`, wrap in try/catch → `DeleteOutcome` (`window-expired`/`not-found`/`network`/`unknown`), never throw; remove from the message-store (FR-027/FR-028; depends on T023, T009, T014)
  - **STATUS (2026-06-15):** code-complete — `deleteMessage(ref)` guards via `requireConnected`, then revokes through the session `RateLimiter` (a revoke IS an outbound send, so it shares the ≤5 msg/min ban-risk budget, FR-016). It uses the genuine cached message `key` when still in the in-session store (the **official Baileys** `{ delete: msg.key }` pattern — verified against baileys.wiki "Modifying Messages": `const msg = await sock.sendMessage(jid, { text }); await sock.sendMessage(jid, { delete: msg.key })`), else reconstructs a Gateway-sent key `{ remoteJid, fromMe: true, id }` (research §161). On success it drops the message from the store and returns `{ ok: true }`; on any WhatsApp/socket rejection it **never throws** — the error is classified by the new **pure, unit-tested** `messages/delete-classifier.ts` (`classifyDeleteError` → `network`/`not-found`/`window-expired`/`unknown` + detail) and returned as `{ ok: false, reason, detail }` (FR-028). Per the implementation discipline the failure-mode mapping was pushed OUT of the untestable shell into a pure unit: `tests/unit/whatsapp-gateway/delete-classifier.test.ts` written FIRST (confirmed RED — module absent), then GREEN (7 assertions). **117 unit tests green**, `tsc --noEmit` + Prettier clean. **Unchecked pending the Scenario F live smoke** (socket-bound; no test account in this env). Flip to `[X]` once Scenario F passes.
- [ ] T045 [P] [US5] Implement `src/whatsapp-gateway/bin/delete-message.ts` entry point (`WA_GROUP_ID`, `WA_MESSAGE_ID`)
  - **STATUS (2026-06-15):** code-complete — reads `WA_CREDS_FILE`/`WA_GROUP_ID`/`WA_MESSAGE_ID` (clear stderr + non-zero exit on missing input), resumes creds, `connect()` → `deleteMessage({ id, groupId })` → prints the `DeleteOutcome` (✅ on `ok:true`, ⚠️ non-fatal on `ok:false`) → `disconnect()` → exits `0`. A rejected revoke prints a clear reason and **does not crash** (FR-028); only true setup/connection failures hit the catch. Imports only `../index.js` (SC-001). `tsc`/Prettier clean. **Unchecked pending the same Scenario F live run as T044.** Run: `WA_CREDS_FILE=./.wa-creds.json WA_GROUP_ID=…@g.us WA_MESSAGE_ID=<id-from-send> npx tsx src/whatsapp-gateway/bin/delete-message.ts`.
