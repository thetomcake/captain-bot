...
- [ ] T003a [P] Add a working ESLint **flat config** (`eslint.config.js`) for ESLint v10 covering `src/**` and `tests/**` with the project's TS rules (strict, no `any`, NodeNext `.js` extensions); migrate/remove the obsolete `.eslintignore`. **Acceptance: `npm run lint` runs to completion and reports cleanly.** The first attempt left lint un-runnable project-wide (no flat config existed), so T049 could not actually be performed — this task makes T049 meaningful (constitution III)
...
- [ ] T049 Run lint/format/typecheck for the new tree (`npm run lint`, `tsc`) — strict, no `any`, `.js` extensions, `#src/*` imports (constitution III)
  - **MUST** actually execute `npm run lint` to completion and pass (requires the flat config from T003a). Do not mark `[X]` if lint cannot run — that is what happened last time
