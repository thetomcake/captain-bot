...
- [X] T016 [P] [US1] Write `tests/unit/whatsapp-gateway/disconnect-classifier.test.ts` FIRST: each `DisconnectReason` code → `restart`/`recover`/`terminal`; `408` (lost/timedOut) → `recover`; `401/403/411/500` → `terminal`; `515` → `restart` (research.md §2)
- [X] T017 [P] [US1] Write `tests/unit/whatsapp-gateway/reconnect-policy.test.ts` FIRST: exponential backoff is bounded/jittered/capped; restart-handshake counter honours `maxRestartHandshakes` (FR-010/FR-011)
- [X] T018 [P] [US1] Write `tests/unit/whatsapp-gateway/credentials.test.ts` FIRST: snapshot round-trip `serialize(state)` → `deserialize(snapshot)` ≡ original, preserving Buffers and v7 key types (`lid-mapping`/`device-list`/`tctoken`) via `BufferJSON` (FR-006/FR-008)
...
- [X] T020 [US1] Implement `src/whatsapp-gateway/connection/reconnect-policy.ts` (backoff schedule + restart-handshake cap); make T017 pass (depends on T017)
- [X] T020a [US1] Implement the **pure connection-state reducer** `src/whatsapp-gateway/connection/connection-state.ts`; make T016a pass. `gateway.ts` (T023) MUST consume this reducer rather than inlining `connection.update` transition logic (depends on T016a, T019, T020)
- [X] T021 [US1] Implement the single source-of-truth credential serialize/deserialize (creds+keys ↔ opaque `WhatsAppCredentials` via `BufferJSON`); make T018 pass (depends on T018, T004)
...
- [X] T022 [US1] Implement `src/whatsapp-gateway/auth/auth-state.ts`: build an in-memory `AuthenticationState` from a snapshot (or `initAuthCreds()`), wrap keys in `makeCacheableSignalKeyStore`, emit `onCredentialsUpdate` on creds/key changes, and `clear()` for re-auth (research.md §1; depends on T021)
...
- [X] T023 [US1] Implement connection lifecycle in `src/whatsapp-gateway/gateway.ts`: create the socket with `getMessage` wired to the **message-store** (returns `undefined` on a miss), `creds.update`→`onCredentialsUpdate`; `connect()` resolves on `open`, absorbs the 515 post-pairing handshake bounded by `maxRestartHandshakes`, auto-reconnects `recover` closes via the reconnect-policy, and surfaces `terminal` closes; tolerate benign `MessageCounterError` at debug (FR-030); implement `disconnect()`, `forceReauth()` (logout + `auth-state.clear()`), `getCredentials()`, and emit `onQR`/`onConnectionChange` (depends on T015, T019, T020, T020a, T022, T014)
...
- [X] T024 [P] [US1] Implement `src/whatsapp-gateway/bin/connect.ts`: read `WA_CREDS_FILE`, pass `credentials` + `onCredentialsUpdate` (writes the file), render QR via `qrcode-terminal`, `connect()`, print status (imports only `../index.js`) (contracts/entry-points.md)
- [X] T025 [P] [US1] Implement `src/whatsapp-gateway/bin/force-reauth.ts`: `forceReauth()` then delete `WA_CREDS_FILE`, print confirmation
...
- [X] T048a [US1] **STOP-AND-VALIDATE gate (do not start US2 until this passes).** Run `quickstart.md` Scenarios A & B against a real test account: (1) first connect shows a QR, scanning reaches `connected`, and the creds file is written; (2) re-run resumes with **no** QR; (3) a brief network kill auto-recovers on the **same** session; (4) `force-reauth` then forces a fresh QR. This is the cheapest check that would have caught the first attempt's blocker defects (C-1 stale creds, C-2 reconnect re-pair). Record the result
  - **STATUS (2026-06-14):** ✅ **GATE PASSED.** Operator ran Scenarios A & B against a real test account: (1) first connect showed a QR, scanning reached `connected`, the creds file was written; (2) re-run resumed with **no** QR; (3) a brief network kill auto-recovered on the **same** session; (4) `force-reauth` then forced a fresh QR. The C-1 (stale creds) and C-2 (reconnect re-pair) blocker defects from the first attempt are confirmed fixed. T023/T024/T025/T048a flipped to `[X]`. (Pure units also green — 67 unit tests, `tsc --noEmit` clean, Prettier clean.)
...
**Goal**: Send text to the authorized group and be notified of genuine inbound messages (sender, text, timestamp), ignoring all other chats and the gateway's own programmatic sends / history backfill (`append`). The linked account is a participant, so the operator's own manual messages (`notify`) ARE reported.

**Independent Test**: `bin/listen.ts` reports a phone-typed group message (including the operator's own — they are a participant) but not a message in another chat nor the gateway's own programmatic (`append`) echoes; `bin/send-message.ts` posts text that appears in the group (quickstart.md Scenario D).

### Tests for User Story 2 (write FIRST, must fail)

- [X] T026 [P] [US2] Write `tests/unit/whatsapp-gateway/message-mapper.test.ts` FIRST: `notify` → `IncomingMessage` (incl. own manual messages — see T027 UPDATE); `append` (own programmatic echo / history) NOT reported as new inbound; text resolved from `conversation` and `extendedTextMessage.text`; timestamp normalized (FR-014/FR-015)

### Implementation for User Story 2

- [X] T027 [US2] Implement `src/whatsapp-gateway/messages/message-mapper.ts` (WAMessage → `IncomingMessage`, notify-vs-append logic, text/timestamp extraction); make T026 pass (depends on T026, T004)
  - **STATUS (2026-06-14):** done — pure unit with `isNewInbound`/`extractText`/`normalizeTimestamp`/`mapIncomingMessage`. Verified the `messages.upsert` `notify`-vs-`append` semantics + text/sender field names against the **official Baileys docs** (baileys.wiki "Receiving Updates" / "Handling Messages"); sender resolved to a canonical `Identity` via the injected `IdentityResolver` (FR-025/FR-026). `tsc --noEmit` + Prettier clean.
  - **UPDATE (2026-06-14):** `isNewInbound` now gates on `type === 'notify'` **only** (no `fromMe` check) — the linked account is a **participant**, so the operator's own manual messages (`notify` + `fromMe`) ARE dispatched; the Gateway's own programmatic sends + history backfill (`append`) stay excluded (FR-015 clarification). Tests updated (notify+fromMe ⇒ dispatched; append+fromMe ⇒ excluded). 82 unit tests green. See spec FR-015 + research.md §2 (observed behaviour). This also makes `listen.ts` solo-testable.
- [X] T028 [US2] Wire receive in `src/whatsapp-gateway/gateway.ts`: on `messages.upsert` store every message (any type) in the message-store, then dispatch only `type==='notify'` items that pass the group-filter through message-mapper + identity-resolver (sender) to `onMessage` handlers (depends on T027, T013, T011, T023, T014)
  - **STATUS (2026-06-14):** code-complete — `handleMessagesUpsert` caches every message unconditionally (backs `getMessage`/poll-secret fast-path), then dispatches `isNewInbound` items (`type === 'notify'` only — own manual messages included; the linked account is a participant) whose `remoteJid` passes `GroupFilter` through `mapIncomingMessage` + the session `IdentityResolver` to `onMessage`; a throwing/rejecting handler can't break the loop. Listener torn down in `teardownSocket` (M-2). `tsc --noEmit` + Prettier clean, 82 unit tests green.
  - **✅ Scenario D passed (2026-06-15):** operator confirmed live — a message typed manually in the authorized group is reported (sender + text); a message in another chat is not reported; cross-chat leakage prevented. Flipped to `[X]`.
- [X] T029 [US2] Implement `sendMessage(groupId, text)` in `src/whatsapp-gateway/gateway.ts`: rate-limited, guards via `requireConnected`, stores the sent message in the message-store (for send-retries), returns `MessageRef` (depends on T007, T009, T023, T014)
  - **STATUS (2026-06-14):** code-complete — guards via `requireConnected` first, sends through the session `RateLimiter` (`sock.sendMessage(groupId, { text })`, `WAMessage | undefined` handled), caches the sent message for retry-receipts, returns `{ id, groupId }`. Send signature verified against research.md §send (installed `7.0.0-rc13`). `tsc`/Prettier clean.
  - **✅ Scenario D passed (2026-06-15):** operator confirmed live — sent text appears in the group and a `MessageRef` prints. Flipped to `[X]`.
- [X] T030 [P] [US2] Implement `src/whatsapp-gateway/bin/send-message.ts` entry point (`WA_GROUP_ID`, `WA_TEXT`)
  - **STATUS (2026-06-14):** code-complete — reads `WA_CREDS_FILE`/`WA_GROUP_ID`/`WA_TEXT` (clear stderr + non-zero exit on missing input), resumes creds, `connect()` → `sendMessage()` → prints the `MessageRef` → `disconnect()` → exits. Imports only `../index.js` (SC-001). `tsc`/Prettier clean. **✅ Scenario D passed (2026-06-15)** — flipped to `[X]`. Run: `WA_CREDS_FILE=./.wa-creds.json WA_GROUP_ID=…@g.us WA_TEXT='hi' npx tsx src/whatsapp-gateway/bin/send-message.ts`.
- [X] T031 [P] [US2] Implement `src/whatsapp-gateway/bin/listen.ts` entry point (long-running; prints inbound)
  - **STATUS (2026-06-14):** code-complete — reads `WA_CREDS_FILE`/`WA_GROUP_ID`, resumes creds, wires `onMessage` to print `sender / text / ts`, connects and stays up (clean SIGINT/SIGTERM shutdown). Cross-chat + `append` (history / own programmatic-send) filtering is enforced in the gateway (T028); own manual `notify` messages are dispatched. Imports only `../index.js` (SC-001). `tsc`/Prettier clean. **✅ Scenario D passed (2026-06-15)** — flipped to `[X]`. Run: `WA_CREDS_FILE=./.wa-creds.json WA_GROUP_ID=…@g.us npx tsx src/whatsapp-gateway/bin/listen.ts`.
...
- [X] T032 [P] [US3] Write `tests/unit/whatsapp-gateway/poll-options.test.ts` FIRST: rejects <2 or >12 options and empty option strings; accepts valid specs (single-choice — no `selectableCount` on the public spec) (FR-020)
- [X] T033 [P] [US3] Write `tests/unit/whatsapp-gateway/poll-tally.test.ts` FIRST for the pure `aggregateVotes(PollVote[])` helper: last-write-per-voter (a later selection replaces the earlier), withdrawal (empty selection) removes the voter, per-option voters canonical with no LID/PN double-count (FR-022/FR-023/FR-026)
- [X] T034 [P] [US3] Write `tests/unit/whatsapp-gateway/poll-vote-decryptor.test.ts` FIRST for the **deterministic** part of the decryptor: a pure option-hash → name mapping that, given a set of decrypted selected option hashes plus an `options` list, returns the correct option names (and ignores an unknown hash). This covers the most-failure-prone capability's testable logic; the `decryptPollVote` crypto boundary itself stays manual-validated (FR-022/FR-031, SC-005)
  - **STATUS (2026-06-15):** all three test files written FIRST and confirmed RED (modules absent), then GREEN after implementation — 26 new assertions across the three files. `poll-vote-decryptor.test.ts` also asserts the **C-3 try-both** control flow (two genuinely distinct attempts: creator `LID` then `PN`, voter + pollMsgId held constant) using an injected `decrypt` seam, plus the pure hash→name mapping (hashes computed in-test via `sha256(Buffer.from(name))` to match installed Baileys).

### Implementation for User Story 3

- [X] T035 [US3] Implement `src/whatsapp-gateway/polls/poll-options.ts` (validate 2–12 non-empty options; single-choice only); make T032 pass (depends on T032)
- [X] T036 [US3] Implement the pure exported helper `aggregateVotes(votes: PollVote[]): PollResult` in `src/whatsapp-gateway/polls/poll-tally.ts` (last-write-per-voter, inverted to per-option voters via identity-resolver); make T033 pass (depends on T033, T011)
  - **STATUS (2026-06-15):** done — pure, stateless. Two passes: learn every LID↔PN pairing carried on the votes via a fresh `IdentityResolver`, then re-key voters by the resolver's stable `canonicalId` (so a person seen as LID then PN collapses to one voter — FR-026), last-write-per-voter, withdrawal (`[]`) removes the voter, inverted to per-option voters in first-appearance order. Exported from `index.ts`. `tsc`/Prettier clean.
- [X] T037 [US3] Implement `src/whatsapp-gateway/polls/poll-vote-decryptor.ts`: a **pure** option-hash → name mapping (make T034 pass), plus `decryptPollVote(vote, { ..., pollEncKey })` taking the resolved secret (raw bytes) + options from the caller, with the #2342 **try-both** creator/voter LID/PN fallback, then mapping selected option hashes → names (verify the exact hashing/aggregation against the installed source — FR-031); return the voter's selected option names, or null on failure (research.md §7; depends on T034, T011)
  ...
  - **STATUS (2026-06-15):** done — `mapOptionHashesToNames` (pure, hex-compared) + `decryptVote(params, deps?)`. **Verified against installed `7.0.0-rc13` (FR-031):** `process-message.js decryptPollVote` builds the sign from `pollMsgId ‖ pollCreatorJid ‖ voterJid ‖ 'Poll Vote' ‖ 0x01`, so a different creator **or voter** form ⇒ genuinely different sign-material. Option hashing replicates `messages.js getAggregateVotesInPollMessage` (`sha256(Buffer.from(optionName))`), compared on hex (robust vs the source's fragile Buffer-vs-Uint8Array `.toString()`).
  - **UPDATE (2026-06-15, matrix fix — live finding):** the original try-both varied **only the creator** (voter held to a single PN-preferred form). Live testing in a **LID group** showed that group encrypts the vote with the **voter in LID form too** (the working self-vote used creator=LID **and** voter=LID; a second account's PN-form attempt failed the auth tag). `decryptVote` now iterates the **full (creator × voter) JID matrix** — `voterCandidates` replaced the single `voterJid` — returning on the first pair GCM accepts (the auth tag rejects wrong pairs, so no false positives). This corrects research §7's earlier "creator=LID, voter=PN" assumption (it's version/group-specific). Unit tests assert both axes are genuinely varied. `tsc`/Prettier clean, **110 unit tests** green. **Manual-validated by Scenario E below.**
- [X] T038 [US3] Implement `sendPoll(groupId, poll)` in `src/whatsapp-gateway/gateway.ts`: validate via poll-options, send the poll with `selectableCount: 1` (single-choice; multi-select out of scope), store the returned poll-creation message in the message-store, read `messageContextInfo.messageSecret` from the result, and return `PollSendResult { ref, keyset }` (keyset = base64 secret + options + ids) (FR-020/FR-021; depends on T035, T029, T014)
  - **STATUS (2026-06-15):** done — guards via `requireConnected`, validates the spec, sends through the session `RateLimiter` as `{ poll: { name, values, selectableCount: 1 } }`, caches the creation message, reads `message.messageContextInfo.messageSecret` (Uint8Array), base64-encodes it into the `PollKeyset`, seeds the in-session secret cache (so our own poll's votes decrypt with no consumer round-trip), and returns `{ ref, keyset }`. **Verified against installed `7.0.0-rc13`:** the poll branch in `messages.js` sets `messageContextInfo.messageSecret` and stores a single-choice poll as `pollCreationMessageV3`. `tsc`/Prettier clean.
  - **✅ Scenario E passed (2026-06-15):** operator confirmed live — the poll appears in the authorized (LID) group, `sendPoll` returns a `MessageRef` + `PollKeyset` (secret written to the keys file). Flipped to `[X]`.
- [X] T039 [US3] Wire vote handling in `src/whatsapp-gateway/gateway.ts`: on an authorized-group `pollUpdateMessage`, build `PollRef`; resolve the secret+options **first from the message-store** (the cached poll-creation message's `messageContextInfo.messageSecret` + `pollCreationMessage.options`), else via `config.resolvePollKeyset(ref)` (null/throw ⇒ skip, no error); decrypt via poll-vote-decryptor, resolve the voter `Identity`, and emit a per-voter `PollVote` via `onPollVote`; cache resolved keysets in-session (FR-021/FR-022/FR-023/FR-024; depends on T037, T011, T028, T014)
  ...
  - **STATUS (2026-06-15):** done — `handleMessagesUpsert` routes a live (`notify`) `pollUpdateMessage` to `handlePollUpdate`, which: relies on the single authorization chokepoint in the caller (the live + authorized-group gate; see research §11 "single chokepoint"); resolves secret+options via `resolvePollSecret` (in-session message store first — reads `messageContextInfo.messageSecret` + option names from `pollCreationMessage`/`V2`/`V3` — then `config.resolvePollKeyset`, caching the result; neither ⇒ skip, no error); builds the **C-3 creator candidates** (`fromMe` ⇒ both `sock.user.lid` + `sock.user.phoneNumber`/`id`) **and voter candidates** (both LID + PN forms), ordered by the cached/lazily-fetched group `addressingMode` (LID-first in a LID group); calls `decryptVote` (full creator × voter matrix — see T037 matrix-fix update); resolves the voter to a canonical `Identity`; emits `PollVote` via `onPollVote` (a throwing handler can't break the loop). `addressingMode` is seeded by `listGroups` and lazily by `ensureAddressingMode`. Per-vote diagnostic form-snapshot logged at debug/warn. `tsc`/Prettier clean.
  - **✅ Scenario E passed (2026-06-15):** operator confirmed live in a **LID-addressed** group — the operator's own votes AND a **second account's** votes both decrypt and attribute correctly (the second account needed the **voter LID** form, which the matrix fix supplies); changing a vote replaces the prior selection (no double-count). This is the exact LID poll-vote path that motivated the rebuild (C-3, FR-024). Flipped to `[X]`.
- [X] T040 [P] [US3] Implement `src/whatsapp-gateway/bin/send-poll.ts`: post the poll and **append the returned `PollKeyset` to `WA_POLL_KEYS_FILE`** (`WA_POLL_QUESTION`, `WA_POLL_OPTIONS`)
  - **STATUS (2026-06-15):** done — reads `WA_CREDS_FILE`/`WA_GROUP_ID`/`WA_POLL_QUESTION`/`WA_POLL_OPTIONS` (comma-separated; clear stderr + non-zero exit on missing input), resumes creds, `connect()` → `sendPoll()`, **appends** the returned `PollKeyset` as one JSON line to `WA_POLL_KEYS_FILE`, prints the `MessageRef`, `disconnect()`, exits. Imports only `../index.js` (SC-001). `tsc`/Prettier clean. Run: `WA_CREDS_FILE=./.wa-creds.json WA_GROUP_ID=…@g.us WA_POLL_QUESTION='Lunch?' WA_POLL_OPTIONS='Pizza,Sushi,Tacos' WA_POLL_KEYS_FILE=./.wa-poll-keys.json npx tsx src/whatsapp-gateway/bin/send-poll.ts`.
  - **✅ Scenario E passed (2026-06-15):** operator confirmed live — poll posts to the group, `MessageRef` printed, `PollKeyset` written to the keys file. Flipped to `[X]`.
- [X] T041 [P] [US3] Implement `src/whatsapp-gateway/bin/watch-votes.ts`: wire `resolvePollKeyset` to read `WA_POLL_KEYS_FILE` (return null when unknown), print each per-voter `PollVote`, and show a running aggregate via `aggregateVotes` (long-running). Note: within one session the in-memory store already serves the secret; the keyset file proves the restart-proof fallback path
  - **STATUS (2026-06-15):** done — long-running; resumes creds, wires `resolvePollKeyset` to read `WA_POLL_KEYS_FILE` (JSON-per-line; latest keyset per `pollId`+`groupId` wins; `null` when unknown), prints each per-voter `PollVote` (voter / selection / poll) and a running per-option tally via the exported `aggregateVotes`, clean SIGINT/SIGTERM shutdown. `WA_DEBUG=1` surfaces the gateway's per-vote form-snapshot diagnostics. Imports only `../index.js` (SC-001). `tsc`/Prettier clean. Run: `WA_CREDS_FILE=./.wa-creds.json WA_GROUP_ID=…@g.us WA_POLL_KEYS_FILE=./.wa-poll-keys.json npx tsx src/whatsapp-gateway/bin/watch-votes.ts`.
  - **✅ Scenario E passed (2026-06-15):** operator confirmed live — per-voter selections print with a running tally; both the operator's and a second account's (LID) votes attribute correctly. Flipped to `[X]`.
...
- [X] T044 [US5] Implement `deleteMessage(ref)` in `src/whatsapp-gateway/gateway.ts`: guards via `requireConnected`; send `{ delete: { remoteJid, fromMe: true, id } }`, wrap in try/catch → `DeleteOutcome` (`window-expired`/`not-found`/`network`/`unknown`), never throw; remove from the message-store (FR-027/FR-028; depends on T023, T009, T014)
  - **STATUS (2026-06-15):** done — `deleteMessage(ref)` guards via `requireConnected`, then revokes through the session `RateLimiter` (a revoke IS an outbound send, so it shares the ≤5 msg/min ban-risk budget, FR-016). It uses the genuine cached message `key` when still in the in-session store (the **official Baileys** `{ delete: msg.key }` pattern — verified against baileys.wiki "Modifying Messages": `const msg = await sock.sendMessage(jid, { text }); await sock.sendMessage(jid, { delete: msg.key })`), else reconstructs a Gateway-sent key `{ remoteJid, fromMe: true, id }` (research §161). On success it drops the message from the store and returns `{ ok: true }`; on any WhatsApp/socket rejection it **never throws** — the error is classified by the **pure, unit-tested** `messages/delete-classifier.ts` and returned as `{ ok: false, reason, detail }` (FR-028).
  - **Revoke is fire-and-forget (verified vs rc13, FR-031):** `relayMessage` awaits only the WS write, never a server ack — so window-expired / message-gone are never thrown (silent server-side). The classifier therefore returns only `network`/`unknown`; `window-expired`/`not-found` are **reserved** in `DeleteOutcome` (documented in `types.ts`, gateway JSDoc, research.md §8, quickstart Scenario F). `{ ok: true }` means "revoke stanza sent", not "confirmed". `delete-classifier.test.ts` written FIRST (RED→GREEN), 9 assertions; **119 unit tests green**, `tsc --noEmit` + Prettier clean.
  - **✅ Scenario F passed (2026-06-15):** operator confirmed live for **both a message and a poll** — a recently-sent item is revoked and disappears from the group (`{ ok: true }`), and the process never crashes. Flipped to `[X]`.
- [X] T045 [P] [US5] Implement `src/whatsapp-gateway/bin/delete-message.ts` entry point (`WA_GROUP_ID`, `WA_MESSAGE_ID`)
  - **STATUS (2026-06-15):** done — reads `WA_CREDS_FILE`/`WA_GROUP_ID`/`WA_MESSAGE_ID` (clear stderr + non-zero exit on missing input), resumes creds, `connect()` → `deleteMessage({ id, groupId })` → prints the `DeleteOutcome` (✅ on `ok:true`, ⚠️ non-fatal on `ok:false`) → `disconnect()` → exits `0`. A rejected revoke prints a clear reason and **does not crash** (FR-028); only true setup/connection failures hit the catch. Imports only `../index.js` (SC-001). `tsc`/Prettier clean. **✅ Scenario F passed (2026-06-15)** — operator deleted both a message and a poll live; flipped to `[X]`. Run: `WA_CREDS_FILE=./.wa-creds.json WA_GROUP_ID=…@g.us WA_MESSAGE_ID=<id-from-send> npx tsx src/whatsapp-gateway/bin/delete-message.ts`.
...
- [X] T046 [P] Write `src/whatsapp-gateway/README.md`: usage example + the full public surface so a developer can integrate without reading Baileys docs (FR-032, SC-008)
  - **STATUS (2026-06-15):** done — `src/whatsapp-gateway/README.md` documents the **entire** public surface (the `WhatsAppGateway` class with a method-by-method behaviour table, all subscriptions, `aggregateVotes`, every domain type, and `GatewayConfig` with defaults), a runnable quick-start, the poll durable-secret pattern, LID/PN identity notes, the `DeleteOutcome` fire-and-forget semantics, and the `bin/` entry-point table with run commands. A consumer can integrate without reading Baileys docs (SC-008). Prettier-clean.
- [X] T047 [P] Audit `src/whatsapp-gateway/index.ts` exports to confirm **no Baileys type leaks** into the public surface (Invariant in contracts/gateway-interface.md)
  - **STATUS (2026-06-15):** PASS — `index.ts` re-exports only `WhatsAppGateway`, the `export type` domain types from `types.ts`, and `aggregateVotes`. Audited: (1) `types.ts`, `index.ts`, and `poll-tally.ts` import **no** Baileys symbol (the only "baileys" token in `types.ts` is a comment); (2) every public `WhatsAppGateway` method signature (`connect`/`disconnect`/`forceReauth`/`isConnected`/`status`/`getCredentials`/`listGroups`/`sendMessage`/`deleteMessage`/`sendPoll`) uses only domain types; (3) the four subscription handler aliases (`QRHandler`/`ConnectionChangeHandler`/`MessageHandler`/`PollVoteHandler`) reference only `string`/`ConnectionStatus`/`IncomingMessage`/`PollVote`; (4) `aggregateVotes(votes: PollVote[]): PollResult`. `tsc --noEmit` is clean, so no implicit Baileys type bleeds through inference. Invariant (contracts/gateway-interface.md) holds.
- [X] T048 Run the unit suite `npx vitest run tests/unit/whatsapp-gateway` — confirm all green and within the fast-suite budget (SC-007)
  - **STATUS (2026-06-15):** PASS — `npx vitest run tests/unit/whatsapp-gateway` → **14 files, 119 tests, all green** in ~2.15s (well within the fast-suite budget, SC-007).
- [X] T049 Run format + typecheck for the new tree (`npm run format:check`, `npx tsc --noEmit`) — strict, no `any`, `.js` extensions, `#src/*` imports (constitution III). (ESLint dropped — see T003a; typecheck via `tsc` is the strictness gate.)
  - **MUST** pass `npx tsc --noEmit` (build `tsconfig.json`) with zero errors and `npm run format:check` cleanly before marking `[X]`
  - **STATUS (2026-06-15):** PASS — `npx tsc --noEmit` exits 0 (zero errors). The new tree is Prettier-clean: `npx prettier --check "src/whatsapp-gateway/**/*.ts" "tests/unit/whatsapp-gateway/**/*.ts"` → "All matched files use Prettier code style!", and the new `README.md` likewise. (Project-wide `npm run format:check` still flags 26 **pre-existing MVP files** outside `src/whatsapp-gateway/` — `src/cli/`, `src/services/`, `src/whatsapp/`, etc. — none in this feature's tree; left untouched per FR-002.)
- [X] T050 Execute `quickstart.md` Part 2 manual validation (Scenarios A–F) against a test account/group and record results
  ...
  - **STATUS (2026-06-15):** ✅ **complete — already validated incrementally during the build, not re-run here.** Per the operator (this phase's instruction), all elements were manually tested as each user story landed: Scenarios A & B at the US1 STOP-AND-VALIDATE gate (T048a, 2026-06-14); Scenario D at US2 (T028–T031); **Scenario E in a LID-addressed group** at US3 (T038/T039/T040/T041 — the operator's own AND a second account's LID-form votes both decrypt and attribute correctly, no double-count on a changed vote — this is the FR-024/C-3 core-motivation path); Scenario C at US4 implicitly (groups listed during every connect); Scenario F at US5 (T044/T045 — message and poll both revoked live). No outstanding manual scenario.
