...
- **`'notify'`** = newly received → route to consumer. **`'append'`** = history/echo (your own sent poll comes back as `'append'`) → **do not report as new inbound** (FR-015). The Gateway dispatches only `'notify'` items as new inbound activity. It keeps a **bounded in-memory cache** of sent/received messages for Baileys send-retries (`getMessage`) and as a poll-secret fast-path; poll-vote decryption falls back to the consumer's keyset when the poll-creation message isn't cached (§7).
  - **Observed behaviour / linked-account participation (2026-06-14):** the Gateway's own **programmatic** sends (`sendMessage`/`sendPoll`) and resync history come back as **`'append'`**, so they are excluded by the notify-only rule. A message the operator sends **manually** from their own phone (same linked account) arrives as **`'notify'` with `key.fromMe: true`** — genuine live inbound activity. Therefore dispatch is gated on **`type === 'notify'` alone, not on `fromMe`** (the operator is a participant; see FR-015 clarification). `IncomingMessage.fromMe` is still recorded so consumers can branch.
...
3. `decryptPollVote(vote, { pollCreatorJid, pollMsgId: pollId, pollEncKey, voterJid })`, where `pollEncKey` is the resolved secret as raw bytes (the store's `messageContextInfo.messageSecret` is already bytes; `base64Decode(keyset.messageSecret)` when it came from the keyset). `pollMsgId` + creator come from `pollCreationMessageKey`; `voterJid` from the vote's key. **LID gotcha (#1678, #2342)** — both `pollCreatorJid` and `voterJid` are mixed into the decryption sign + GCM AAD, so the form of *each* must match what the voting client used.
   - **CORRECTED at implementation time (2026-06-15, live LID-group testing — supersedes the earlier `creator=LID, voter=PN` guess):** the addressing form is **not** fixed and is **not** the same on both axes across versions/clients. Observed in a real **LID-addressed** group: a vote was encrypted with **creator = LID *and* voter = LID** (a second account's vote failed when we tried only the voter's PN form). The reliable approach is to try the **full (creator × voter) JID matrix** — `creatorCandidates ∈ {our LID, our PN}` (for our own poll) × `voterCandidates ∈ {voter LID, voter PN}`, LID-first in a LID group — wrapping each attempt in try/catch and returning on the first pair that decrypts. AES-GCM's auth tag rejects every wrong pair, so iterating the product is safe (no false positives). Identify our own creator identity via `pollCreationMessageKey.fromMe` + `sock.user.lid`/`sock.user.phoneNumber`; derive both voter forms from the vote key's `participant`/`participantAlt`.
...
**Decision (`group-filter.ts`, FR-017/FR-018)**: filter `messages.upsert` (`type === 'notify'` only) and any vote handling by `remoteJid ∈ authorizedGroups`, guarded with `isJidGroup`. Explicitly ignore `status@broadcast`, newsletters, and DMs.

**Implementation refinement (2026-06-15, single chokepoint)**: the live (`notify`) + authorized-group gate is applied in **exactly one place** — `gateway.ts handleMessagesUpsert`, on the upsert message's own `remoteJid`, **before** routing to the text-dispatch or poll-vote handler. Downstream handlers (incl. `handlePollUpdate`) **trust** this gate and do **not** re-filter, so the FR-017/SC-004 zero-leakage invariant has one enforcement point rather than several (less risk a future handler forgets it). This is safe for poll votes because a `pollUpdateMessage` always arrives **in the poll's own chat**, so the vote message's `remoteJid` equals the poll-creation message's group. The poll-creation message's `remoteJid` is still used as the *source of the `PollRef`/`PollVote.groupId` identifier* (the original §11 intent) — it is just no longer a second authorization filter.
