...
**`getMessage` config requirement (`lib/Types/Socket.d.ts`)**: `getMessage: (key) => Promise<proto.IMessage | undefined>`. Baileys calls it to resend messages on poor connections and (historically) for poll decryption. Wire it to an in-memory best-effort cache of recently-sent messages for **send-retries**. Note: in our design poll-vote decryption does **not** depend on `getMessage` or on retaining the poll-creation message — the per-poll `messageSecret` comes from the consumer's keyset (§6, §7). It is fine for `getMessage` to return `undefined` after a restart.
...
- **`'notify'`** = newly received → route to consumer. **`'append'`** = history/echo (your own sent poll comes back as `'append'`) → **do not report as new inbound** (FR-015). The MVP already learned this the hard way; the Gateway dispatches only `'notify'` items as new inbound activity. It may keep a small in-memory cache of recent messages for Baileys send-retries (`getMessage`), but poll-vote decryption no longer depends on retaining the poll-creation message — the consumer supplies the keyset (§7).
...
- `selectableCount`: `1` = single-choice; `0`/omit ≈ multi. Validate **2–12 options** in `poll-options.ts` before sending (WhatsApp client caps at 12; Baileys does not enforce).
...
- **The Gateway captures the secret into the keyset (storage-agnostic):** after sending, read `messageContextInfo.messageSecret` from the returned message, base64-encode it, and return it — with `pollId`, `groupId`, `options`, `selectableCount` — as the **`PollKeyset`** from `sendPoll` (FR-020/FR-021). The **consumer persists the keyset** and hands it back via `resolvePollKeyset` when a vote arrives (§7). The Gateway does **not** retain the poll-creation `WAMessage` for decryption (a short-lived in-memory copy for send-retries may exist, but it is not on the vote path and may be lost on restart).
...
**Implication for the Gateway**: doing decryption ourselves is now the *intended* v7 pattern, not a workaround. Our `poll-vote-decryptor.ts` implements exactly the recommended flow below, with the #2342 LID/PN try-both fallback, and must store the poll-creation message so the `messageSecret` is always recoverable.
...
2. Build `PollRef { pollId, groupId }`; call `resolvePollKeyset(ref)`. `null` or throw ⇒ skip (debug-log), never error (FR-021).
3. `decryptPollVote(vote, { pollCreatorJid, pollMsgId: pollId, pollEncKey: base64Decode(keyset.messageSecret), voterJid })`. `pollMsgId` + creator come from `pollCreationMessageKey`; `voterJid` from the vote's key. **LID gotcha (#1678, #2342)**: try `creator = normalized LID, voter = PN`; on throw, retry `creator = PN, voter = PN` (use `pollCreationMessageKey.fromMe` + `sock.user.lid` to identify our own creator identity). Wrap each attempt in try/catch.
4. Map decrypted selected option **hashes → names** using `keyset.options` (this replicates Baileys' own option-hash matching). **Verify the exact hashing/aggregation call against the installed source at implementation time** (FR-031): either hash `keyset.options` ourselves, or synthesize the minimal creation-message object the exported `getAggregateVotesInPollMessage` expects from the keyset. We deliberately do **not** rely on a stored creation `WAMessage`.
...
**`getMessage`**: not required for poll decryption in this design — the `messageSecret` comes from the consumer's keyset, not a stored message. Keep `getMessage` wired only for Baileys send-retries (§2); it may return `undefined`.
