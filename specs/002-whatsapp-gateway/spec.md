...
- **Poll-vote decryption is version-sensitive and storage-driven**: automatic poll-vote handling differs between protocol-library versions and is disabled in the pinned version, so the Gateway decrypts votes itself; because it persists nothing, it obtains the per-poll decryption secret from the consumer (returned by `sendPoll`, supplied back via a resolver). A vote whose keyset the consumer cannot supply is skipped, not errored.
...
- **FR-020**: The Gateway MUST post a native WhatsApp poll to a configured group given a question, a set of options (2–12), and how many options a voter may select, and MUST return a reference for the poll together with its poll keyset (FR-021).
- **FR-021**: Because the Gateway persists nothing, `sendPoll` MUST return a **poll keyset** — the decryption secret plus the poll's options (and ids) — for the consumer to store. To decrypt a later vote the Gateway MUST request that keyset from the consumer via a resolver callback keyed by the poll reference; if the consumer returns no keyset, the Gateway MUST skip decryption for that vote without error (best-effort, never crash).
...
- **Group**: a WhatsApp group, identified by a stable identifier and a display name; some are authorized, the rest are ignored.
...
- **Identity**: the canonical representation of a person, reconciling the multiple address forms (JID/LID, with/without device suffix) WhatsApp may use for the same human.
